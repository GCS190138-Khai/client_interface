...
function CheckOut() {
  ...
  useEffect(()=>{
    const handleAddCity= async()=>{ 
      ...
      const res = await axios.get('https://phobendoi.art/api/city/getShipments')
   ...
   if(!currentUser){
    ...
    if ( window.confirm(text) === true) {
     ...
     let code ={
      ...
      try {
        const res = await axios.post('https://phobendoi.art/api/discount/check',code)
    ...
    if(!checkCode){
      ...
      if(res.data[0].forWhat==="event"){
        ...
        if(!check){
          ...
          setIsCheck(true)
          try {
            const res = await axios.post('https://phobendoi.art/api/products/bill',newBill)
...
handleSubmitForm(e)
        ...
        <div className=" gap-[80px]   flex flex-col ">
          ...
          <div className=" gap-[10px] items-center text-aCaption font-[400] uppercase flex">
            <span className=" opacity-50">giỏ hàng</span>
