...
import {  getAllEventAdmin, getEventById } from "../api"
...
function AllEventsAdmin() {
  ...
  useEffect(()=>{
    if(isLoading){
      (async () => {
        await     getAllEventAdmin(dispatch)
    ...
    <div className=" h-auto w-[100%] flex flex-col gap-[12vh]  ">
      {
        list.map((item,i)=>{
          return (
            ...
            <Link key={i} onClick={()=>{ getEventById(dispatch,item._id)   }} state={{tab : "tất cả",event:item,}}  to={`/adminPhobendoi/${item._id}`} >
